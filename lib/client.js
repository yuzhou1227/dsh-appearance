...
function save(s) {
  ...
  // Mirror every write to the host settings document so the choice survives
  // desktop restarts (the sandboxed renderer's localStorage is ephemeral).
  try {
    fetch('/api/dsh-appearance/set', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ data: JSON.stringify(s) })
    }).catch(function (err) {
      console.log('appearance: host persist failed:', err && err.message);
    });
  } catch (e) { /* host unreachable */ }
}

function hydrateFromHost(ctx, attempt) {
  attempt = attempt || 0;
  try {
    fetch('/api/dsh-appearance/get', { cache: 'no-store' }).then(function (r) {
      if (!r.ok) throw new Error('HTTP ' + r.status);
      return r.json();
    }).then(function (body) {
      if (!body || typeof body.data !== 'string' || body.data === '') {
        console.log('appearance: host has no stored settings yet');
        return;
      }
      ...
      console.log('appearance: hydrated from host settings:', JSON.stringify(next));
    }).catch(function (err) {
      console.log('appearance: host hydrate failed (attempt ' + attempt + '):', err && err.message);
      // The host route may register after this bundle loads; retry a few times.
      if (attempt < 10) setTimeout(function () { hydrateFromHost(ctx, attempt + 1); }, 1500);
    });
  } catch (e) { /* ignore */ }
}
...
function apply(ctx) {
  ...
  console.log('appearance: static bundle v4 active, palettes light=' + LIGHT_PALETTES.length + ' dark=' + DARK_PALETTES.length);
  try {
    console.log('appearance: loaded from storage:', JSON.stringify(load()));
  } catch (e) {
    console.log('appearance: storage read failed:', e.message);
  }
  ...
  function applyWhenReady() {
    ...
    readyTries += 1;
    if (readyTries < 400) setTimeout(applyWhenReady, 60);
  }
  applyWhenReady();
  ...
  function AppearancePage() {
    ...
    React.useEffect(function () {
      // localStorage is the single source of truth. Applying the host theme
      // preference back here would override the user's stored choice with
      // the default "system" on every boot.
      const next = load();
      applySettings(ctx, next);
      setS(next);
