...
function hydrateFromHost(ctx) {
  try {
    fetch('/api/dsh-appearance/get', { cache: 'no-store' }).then(function (r) {
      ...
      console.log('appearance: hydrated from host settings');
    }).catch(function (err) {
      console.log('appearance: host hydrate failed:', err && err.message);
    });
  } catch (e) { /* ignore */ }
}
...
function apply(ctx) {
  ...
  console.log('appearance: static bundle v3 active, palettes light=' + LIGHT_PALETTES.length + ' dark=' + DARK_PALETTES.length);
  try {
